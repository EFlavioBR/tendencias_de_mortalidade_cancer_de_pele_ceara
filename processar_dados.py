...
# Rejeita anos fora do período de análise (2011-2021) para evitar registros com
# datas de digitação tardia ou erros de preenchimento comuns nas bases do DATASUS
def extrair_ano(dt):
    try:
        dt_str = str(int(dt)).zfill(8)  
        ano = int(dt_str[-4:])          
        if 2011 <= ano <= 2021:
            return ano
        return np.nan                   
...
for arquivo in arquivos_csv:
    ...
    if not df_pele.empty:
        ...
        # Filtro explícito: mantém apenas registros dentro do período de análise
        df_pele = df_pele[df_pele['Ano_Obito'].between(2011, 2021)]

        # Pula para o próximo arquivo se não houver dados no período
        if df_pele.empty:
            print(f"  → Nenhum registro no período 2011-2021 em {nome_arquivo}. Pulando.")
            continue
...
if len(lista_dados) > 0:
    ...
    # Confirmação do período no arquivo final
    print(f"\nPeríodo dos dados: {int(df_final['Ano_Obito'].min())} a {int(df_final['Ano_Obito'].max())}")
    ...
    print(f"Sucesso! Foram unidos {len(df_final)} óbitos de pele.")
